...
class Plugin extends AbstractPlugin implements PaymentInterface, PaymentQueryInterface
{
    ...
    public function query(string $tradeNo): array|bool
    {
        try {
            $gateway = new AlipayF2F();
            $gateway->setMethod('alipay.trade.query');
            $gateway->setAppId($this->getConfig('app_id'));
            $gateway->setPrivateKey($this->getConfig('private_key'));
            $gateway->setAlipayPublicKey($this->getConfig('public_key'));
            $gateway->setBizContent([
            ...
            $paid = isset($response['trade_status']) && $response['trade_status'] === 'TRADE_SUCCESS';
            return [
                'paid' => $paid,
                'callback_no' => $response['trade_no'] ?? null,
                'raw' => $response
            ];
        } catch (\Exception $e) {
            Log::error($e);
